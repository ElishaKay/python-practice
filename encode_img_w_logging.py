...
python steganography.py -f penguin.bmp -o Y.bmp -m "Hidden message!"
'''
...
def set_bit(v, index, x):
  ...
  print('These are the params of the set_bit function before the masking. v: ',v,'index: ', index,'x: ', x)
  mask = 1 << index
  print('after conversion, mask = ',mask)
  v = v&(~mask)
  # v &= ~mask
  if x:
    v = v|mask
...
def setlsb(component, bit):
    print('ran setlsb',component, bit)
    """
...
def loopThroughNewImagePixels(input_image_file):
...
def hide(input_image_file, message, auto_convert_rgb=False, order_func = fib):
    ...
    """
    print('order_func: ',order_func)
    print('input_image_file, message :',input_image_file, message)
    ...
    print('this is the message_bits after 3 actions: ',message_bits)
    
    npixels = width * height
    print('npixels: ',npixels)
    len_message_bits = len(message_bits)
    print('len_message_bits: ',len_message_bits)
    ...
    for row in range(height):
        for col in range(width):
            if index + 3 <= len_message_bits:
                ...
                g = pixel[1]
                b = pixel[2]
                print('within rgb loop of hide function, before the ran_set_bit conversions, the pixels = ',pixel)
